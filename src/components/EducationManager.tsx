...
export default function EducationManager({ currentEducation, dict }: EducationManagerProps) {
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      try {
        ...
        if (file) {
          if (file.size > 5 * 1024 * 1024) throw new Error(dict.fileTooLarge);
        ...
        setStatus({ type: "error", message: error.message || dict.generalError });
      } finally {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200 mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 bg-slate-50 p-4 md:p-6 rounded-2xl border border-slate-100">
        <input
          type="text" placeholder={dict.degreePlaceholder}
        ...
        />
        <input
          type="text" placeholder={dict.institutionPlaceholder}
        ...
        <div className="md:col-span-2 pt-4 border-t border-slate-200 mt-2">
          <label className="block text-sm font-bold text-slate-700 mb-2">{dict.uploadLabel}</label>
          ...
          {file ? (
            ...
            <label className="relative flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 border-dashed rounded-2xl cursor-pointer bg-white hover:bg-slate-50 hover:border-indigo-400 transition-all group">
              <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center">
                ...
                <p className="text-sm text-slate-600 font-bold">{dict.clickToUpload}</p>
                <p className="text-xs text-slate-400">{dict.uploadConstraints}</p>
              </div>
              <input 
        ...
        >
          {isPending || isUploading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Plus className="w-5 h-5 me-2" /> {dict.saveButton}</>}
        </button>
      </div>
      ...
      <div className="space-y-3">
        {currentEducation.length > 0 ? (
            ...
            <div key={edu.id} className="flex justify-between items-center p-4 border-2 border-slate-100 rounded-xl bg-white hover:border-slate-200 transition-colors">
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium flex items-center gap-2">
                  {edu.institution} <span className="text-slate-300">•</span> {edu.startYear} - {edu.endYear || dict.present}
                  {edu.documentUrl && <span className="bg-emerald-50 text-emerald-600 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wide border border-emerald-100">{dict.verified}</span>}
                </p>
              </div>
          ...
          <p className="text-center text-sm font-medium text-slate-400 py-4 italic bg-slate-50 rounded-xl border border-dashed border-slate-200">{dict.noEducation}</p>
        )}
      </div>
    </div>
  );
